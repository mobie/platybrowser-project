...
def training(project_directory,
    ...
    train_loader = get_platyneris_loader(data_configuration_file, lazy=True)

    logger.info("Loading validation data loader from %s." % validation_configuration_file)
    validation_loader = get_platyneris_loader(validation_configuration_file, lazy=True)
...
def make_train_config(train_config_file, gpus, affinity_config):
    template = './template_config/train_config.yml'
