...
def set_up_training(project_directory,
    ...
    affinity_config = data_config['volume_config']['segmentation'].get('affinity_config', None)

    loss = LossWrapper(criterion=SorensenDiceLoss(),
                       transforms=Compose(ApplyAndRemoveMask(), InvertTarget()))

    drop_binary = DropChannels(index=0, from_='prediction')
    loss_val = LossWrapper(criterion=SorensenDiceLoss(),
                           transforms=Compose(drop_binary, RemoveSegmentationFromTarget(),
                                              ApplyAndRemoveMask(), InvertTarget()))

    offsets = affinity_config['offsets']
    strides = [4, 4, 4]
    metric = LossWrapper(criterion=ArandErrorFromMWS(offsets=offsets, strides=strides),
                         transforms=drop_binary)
...
def make_data_config(template_config, data_config_file, n_batches, affinity_config=None):
    ...
    affinity_config.update({'segmentation_to_binary': True, 'retain_segmentation': False})
    template['volume_config']['segmentation']['affinity_config'] = affinity_config
    template['volume_config']['segmentation']['dtype'] = 'int64'

    template['volume_config']['segmentation']['is_multichannel'] = False
    key = 'volumes/labels/' + key
    for block_id in range(1, 13):
        template['volume_config']['segmentation']['path_in_file'][str(block_id)] = key

    with open(data_config_file, 'w') as f:
        yaml.dump(template, f)


def make_validation_config(template_config, validation_config_file, affinity_config=None):
    template = os.path.join(template_config, 'validation_config.yml')
    template = yaml2dict(template)

    key = 'nucleus_instance_labels'
    affinity_config.update({'segmentation_to_binary': False, 'retain_segmentation': True})
    template['volume_config']['segmentation']['affinity_config'] = affinity_config
    template['volume_config']['segmentation']['dtype'] = 'int64'
...
def main():
    ...
    affinity_config = {'retain_mask': True, 'ignore_label': -1}
    offsets = get_offsets()
